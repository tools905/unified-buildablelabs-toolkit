...
export function LinkedInScoringExplainer() {
  return <Card>
    ...
    <CardContent className="space-y-4 text-sm text-muted-foreground">
      ...
      <Explanation title="Mock test data">A deterministic development connector that creates synthetic activities. It is useful for testing the complete workflow but must not be used as real member performance data.</Explanation>
      <Explanation title="Fallback collector">A reserved connector boundary for a future compliant backup collection method. It is not configured today and sync attempts fail visibly instead of returning fake success.</Explanation>
      <Explanation title="Third-party API">The production-provider boundary intended for a service such as Apify. It remains unavailable until provider credentials and the fetch adapter are implemented.</Explanation>
    </CardContent>
  </Card>;
}
