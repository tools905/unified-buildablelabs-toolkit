...
describe("LinkedIn Assessor", () => {
  ...
  it("combines volume and quality using member weights", () => {
    const stats = calculateLinkedInMemberStats({
      member: {
        ...
        volume_weight: 0.5, quality_weight: 0.5, tracking_status: "active", is_active: true, created_at: new Date().toISOString(),
      },
      posts: [{
  ...
  it("validates manual submissions and rolling window bounds", () => {
    ...
    expect(() => linkedinSettingsSchema.parse({ monthlyPostTarget: 12, volumeWeight: 0.45, qualityWeight: 0.55, weeklyReportsEnabled: true, memberInsightsEnabled: true, memberSubmissionsEnabled: true, analysisWindowDays: 3 })).toThrow();
    expect(linkedinSettingsSchema.parse({ monthlyPostTarget: 12, volumeWeight: 0.45, qualityWeight: 0.55, weeklyReportsEnabled: true, memberInsightsEnabled: true, memberSubmissionsEnabled: true, analysisWindowDays: 30 }).analysisWindowDays).toBe(30);
  });
