  ...
  const feed = (posts ?? []).map((post) => {
    ...
    return {
      ...
      scoreBreakdown: score ? {
        ...
        engagement: Number(score.engagement_score), hashtags: Number(score.hashtag_score ?? 0), writingQuality: Number(score.writing_quality_score),
      } : null,
  ...
  if (input.notifyMember && member) {
    ...
    if (recipient) {
      await sendLinkedInPostSummaryEmail(supabase, {
        ...
        qualityBreakdown: {
          hook: Number(result.score.hook_score),
          hashtags: Number(result.score.hashtag_score),
          originality: Number(result.score.originality_score),
          writingQuality: Number(result.score.writing_quality_score),
        },
